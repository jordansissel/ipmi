...
struct rmcp getChannelAuthenticationCapabilities(IPMI::AuthenticationCapability authCap) {
    ...
    r.session.message.parameters.GetChannelAuthenticationCapabilities.channel = 0x0e; /* Use current channel */
    r.session.message.parameters.GetChannelAuthenticationCapabilities.privileges = (uint8_t)authCap; /* Request Administrator privileges */
    r.session.message.parameters.GetChannelAuthenticationCapabilities.checksum = -( 
        + r.session.message.source
        + (r.session.message.sequence << 2 | r.session.message.sourceLun)
        + r.session.message.command
        + r.session.message.parameters.GetChannelAuthenticationCapabilities.channel 
        + r.session.message.parameters.GetChannelAuthenticationCapabilities.privileges
    );
